...
if(exists("snakemake")){
  ...
  regions <- snakemake@wildcards$region
...
datas[[1]] <- datas[[1]] %>% filter(library_id == sample, seurat_clusters %in% clusters)
cat('Unique seurat cluster IDs:', paste(as.character(sort(unique(datas[[1]]$seurat_clusters))), collapse = ', '), '\n')
...
cat('Number of spots: ', nrow(datas[[1]]), '\n')
