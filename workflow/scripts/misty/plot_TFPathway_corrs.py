...
if 'snakemake' in locals():
    ...
    ct_annot_fp = snakemake.input[6]
    ttests_fp = snakemake.input[7]
    corrs_fp = snakemake.input[8]
    ...
    ttests_fp = 'results/Misty/{0}/interactions_TFPathway_ttests.csv'.format(tissue)
    corrs_fp = 'results/Misty/{0}/interactions_TFPathway_corrs.csv'.format(tissue)
...
# %%
ct_annot = pd.read_csv(ct_annot_fp)
...
# %%
ttests = pd.read_csv(ttests_fp, index_col=0)
corrs = pd.read_csv(corrs_fp, index_col=0)
...
for index, row in sig_associations.iterrows():
    ...
    for mouse, plot_column in zip([flight_mouse, ground_mouse], [1,3]):
        ...
        sc.pl.spatial(acts, img_key=None, library_id=mouse, \
