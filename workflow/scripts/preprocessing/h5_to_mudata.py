...
if 'snakemake' in locals():
    ...
    input_file = 'results/MO/brain_test.h5'
...
def readh5_to_anndata(file, main_assay = 'RNA'):
    ...
    with h5py.File(file, "r") as f:
        ...
        if(X.attrs['datatype'] == 'SparseMatrix'):
            mat = csr_matrix((X['values'], X['indices'], X['indptr']), shape=(X['dims'][0], X['dims'][1]))
        else:
            mat = np.array([X['matrix'][key][()] for key in X['matrix'].keys()], dtype=np.float64)
...
# %%
adata = readh5_to_anndata(input_file, main_assay = 'RNA')

atac = readh5_to_anndata(input_file, main_assay = 'ATAC')

chromvar = readh5_to_anndata(input_file, main_assay = 'chromvar')

mdata = mu.MuData({"rna": adata, "atac": atac, "chromvar": chromvar})
